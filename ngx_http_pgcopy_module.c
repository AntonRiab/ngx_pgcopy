...
ngx_int_t ngx_http_pgcopy_init_access_handler(ngx_http_request_t *r);

ngx_int_t ngx_pgcopy_start_pooling(ngx_http_request_t *r);
ngx_int_t ngx_pgcopy_need_made(ngx_http_request_t *r);
ngx_int_t ngx_http_pgcopy_access_handler(ngx_http_request_t *r);
...
void  ngx_pgcopy_upstream_free_peer_begin_connect(ngx_peer_connection_t *pc, 
                                                  void *data,
                                                  ngx_uint_t state);
...
void ngx_pgcopy_force_finalize(ngx_http_request_t *r, ngx_int_t rc);
...
void ngx_pgcopy_query_handler(ngx_event_t *ev);
void ngx_pgcopy_body_ok(ngx_http_request_t *r);
...
ngx_http_pgcopy_srv_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_list_init(&conf->conn_info_list, cf->pool, 1, sizeof(connection_info_srv)) 
            == NGX_ERROR) {
...
#define CAPTURE_LEN 2
    ...
    if (cf->args->nelts == 4) {
        if (!ngx_strncasecmp((u_char*)"basic", value[3].data, value[3].len)) {
            ...
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, 
                               "Authentification not recognized: \"%s\" - ERROR!", value[3].data);
            return NGX_CONF_ERROR;
        }
    }
    ...
    if (ngx_regex_exec(rc.regex, &conn_element->conn_info, (int*)captures, 3*CAPTURE_LEN) 
        ...
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "PGCOPY: not found hostname in \"%V\" !",
            &cmd->name);
        return NGX_CONF_ERROR;
    }
#undef CAPTURE_LEN
...
#ifdef PGCOPY_DEBUG
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "<pgcopy_server connection_name=\"%V\">",
        &conn_element->conn_name);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "    Host addr      : \"%V\"",
        &conn_element->conn_host);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "    Connection info: \"%V\"",
        &conn_element->conn_info);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "    Auth type str  : \"%V\"", &value[3]);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "    Auth type enum : \"%i\"",
        conn_element->auth_type);
...
ngx_http_conf_pgcopy_query(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    current_conn_element->cf = cf;

    ngx_memzero(&current_conn_element->ns_compile, sizeof(ngx_http_script_compile_t));

    current_conn_element->ns_compile.variables = 
                        ngx_http_script_variables_count(&current_conn_element->pgquery);
    ...
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
        "    Conn_name      : \"%V\"", &current_conn_element->conn_inf_srv->conn_name);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
        "    Conn_info      : \"%V\"", &current_conn_element->conn_inf_srv->conn_info);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
        "    Conn_authon.   : \"%i\"", current_conn_element->conn_inf_srv->auth_type);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
        "    Conn_query     : \"%V\"", &current_conn_element->pgquery);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
        "    Nginx_script   : \"%i\"", current_conn_element->ns_compile.variables);
    ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "</pgcopy_query>");
#endif
...
ngx_http_pgcopy_init_access_handler(ngx_http_request_t *r)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, r->connection->log, 0, 
        "PGCOPY: <info full_conn_info=\"%s\"/>", (char*)full_conn_info);
...
ngx_pgcopy_start_pooling(ngx_http_request_t *r)
{
    ...
    switch (ctx->status) 
    {
        case PGRES_POLLING_FAILED:
            PGCOPY_DTRACE1(r->connection->log, "PGCOPY PGRES_POLLING_FAILED! Process connect %s", 
                PQresStatus(ctx->status));
            if (ngx_strncasecmp((u_char*)"FATAL:  password authentication failed",                  
                                (u_char*)PQerrorMessage(ctx->pgconn),
                                sizeof("FATAL:  password authentication failed"))){
                PGCOPY_DTRACE(r->connection->log, "PGCOPY password authentication failed!");
                if (ctx->current_loc_conninfo->conn_inf_srv->auth_type) {
                    ngx_http_finalize_request(r, ngx_http_auth_basic_set_realm(r, &
                        (ngx_str_t)ngx_string("Unauthorised request")));
                } else {
        ...
        case PGRES_POLLING_OK:
            PGCOPY_DTRACE1(r->connection->log,
                "PGCOPY: <process connect='PGRES_POLLING_OK %s/>", PQresStatus(ctx->status));
            break;
        default:
            PGCOPY_DTRACE1(r->connection->log, "PGCOPY: <process connect='default %s'/>",
                PQresStatus(ctx->status));
            PGCOPY_DTRACE(r->connection->log, "PGCOPY: </ngx_pgcopy_start_pooling>");
            return NGX_AGAIN;
    }
...
ngx_pgcopy_need_made(ngx_http_request_t *r)
{
    ...
    switch(PQstatus(ctx->pgconn))
    {
        ...
        case CONNECTION_MADE:
            PGCOPY_DTRACE(r->connection->log,
                "PGCOPY: <connect_stage status=\"connected, need made or waiting to send...add timer\"");
            PGCOPY_DTRACE(r->connection->log, "PGCOPY: </ngx_pgcopy_need_made>");
            return NGX_AGAIN;
        default:
...
ngx_http_pgcopy_access_handler(ngx_http_request_t *r)
{
    ...
    switch(rc) {
        case NGX_OK:
            r->state = 0;
            r->content_handler = ngx_pgcopy_content_handler; //hz, may be
            break;
        case NGX_AGAIN:
            r->phase_handler--;
            break;
...
ngx_pgcopy_upstream_init(ngx_http_request_t *r, ngx_http_pgcopy_ctx_t *ctx)
{
    ...
    uscf->peer.init = ngx_pgcopy_upstream_init_peer;//without seg fault
    ...
    u->conf->send_timeout = 5000;


    r->state = 0;
    u->peer.get = ngx_pgcopy_upstream_get_peer_begin_connect;
    u->peer.free = ngx_pgcopy_upstream_free_peer_begin_connect;
    u->peer.data = r;
    u->peer.tries=10;
...
ngx_pgcopy_upstream_get_peer_begin_connect(ngx_peer_connection_t *pc, void *data)
{
    ...
    //pc->connection->addr_text.data = ngx_pcalloc(r->pool, sizeof("1"));
    //pc->connection->addr_text.len = 1;
...
void
ngx_pgcopy_upstream_free_peer_begin_connect(ngx_peer_connection_t *pc, 
                                            void *data, 
                                            ngx_uint_t state)
{
    PGCOPY_DTRACE(pc->log, "PGCOPY: </ngx_pgcopy_upstream_free_peer_begin_connect>");
...
ngx_pgcopy_in(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    if (PQisBusy(ctx->pgconn)) {
        PGCOPY_DTRACE(r->connection->log, "PGCOPY: <busy action=\"return\"/>");
//        if( !ctx->pc->connection->write->timer_set ) 
//            ngx_add_timer(ctx->pc->connection->write, 100);//timeout
        return;
    }
    ...
    if (r->request_body->temp_file) {
        ctx->n = ngx_read_file(&r->request_body->temp_file->file, 
                               ctx->current_buffer, ctx->client_body_buffer_size, ctx->offset);
        ctx->offset += ctx->n;
    } else if (ctx->cl){
    ...
    if (ctx->n > 0){
        PGCOPY_DTRACE2(r->connection->log, 
                       "PGCOPY: <begin_data_output/>\n<data size=\"%i\">\n%s\n</data>",
                       ctx->n, ctx->current_buffer);
    ...
    if (!r->request_body->rest && ctx->n < 1) {
        ...
        ngx_pgcopy_force_finalize(r, NGX_DONE);
    ...
    if (r->request_body->rest || (ctx->n > 0) ) {
        ngx_log_debug0(NGX_LOG_WARN, r->connection->log, 0, "PGCOPY: <ngx_pgcopy_in TIMTER/>");
        if (!ctx->pc->connection->write->timer_set) {
...
ngx_pgcopy_out(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
        ...
        if (ctx->n < 0) {
            ...
            if (ctx->cl->buf->end < buff_last_next) {
                if (!ctx->pc->connection->read->timer_set) {
                    ngx_add_timer(ctx->pc->connection->read, 1);
                }
                break;
            }
    ...
    if (ctx->n < 0) {
        ...
        //How to close chunked without send any data, ngx_http_chunked_header_filter
        ctx->cl->buf->pos = ctx->cl->buf->start;
        ctx->cl->buf->last = ctx->cl->buf->start+1;
        *ctx->cl->buf->start = '\n';
...
void
ngx_pgcopy_force_finalize(ngx_http_request_t *r, ngx_int_t rc) {

    PGCOPY_DTRACE(r->connection->log, "PGCOPY: <ngx_pgcopy_force_finalize>");
...
ngx_pgcopy_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
    ...
    if (PQresultStatus(ctx->pgres) == PGRES_FATAL_ERROR) {
            ngx_log_debug1(NGX_LOG_WARN, r->connection->log, 0, 
                           "PGCOPY: PQgetResult ERROR status: %s\n",
...
ngx_pgcopy_content_handler(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_read_client_request_body(r, ngx_pgcopy_body_ok);
...
void
ngx_pgcopy_body_ok(ngx_http_request_t *r)
{
    ngx_http_pgcopy_ctx_t     *ctx;
    ngx_str_t                  sc_pquery;
    connection_info_loc       *current_loc_conninfo;
    ...
    PGCOPY_DTRACE(ctx->request->connection->log, "PGCOPY: <ngx_pgcopy_body_ok>");

    current_loc_conninfo = ctx->current_loc_conninfo;

    if (current_loc_conninfo->ns_compile.variables > 0) {
        PGCOPY_DTRACE(r->connection->log, "PGCOPY: <script_run>");

        if (ngx_http_script_run(r, &sc_pquery, current_loc_conninfo->ns_lengths->elts, 0,
                                current_loc_conninfo->ns_values->elts) == NULL) {
            return;
        }
